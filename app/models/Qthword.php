...
class Qthword extends BaseThword {
    ...
    public function findRandom() {
        ...
        $sql = "SELECT t.*
                FROM (SELECT ROUND(RAND() * (SELECT MAX(id) FROM thw_qthwords)) num, @num:=@num+1 FROM (SELECT @num:=0) AS a, thw_qthwords LIMIT 1) AS b,
                thw_qthwords AS t
                WHERE b.num = t.id";
